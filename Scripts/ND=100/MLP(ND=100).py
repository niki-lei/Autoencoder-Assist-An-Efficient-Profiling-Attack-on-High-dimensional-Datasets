...
def train_model(X_profiling, Y_profiling, model, save_file_name, epochs=150, batch_size=100,node=16,layer_nb=3,input_dim=60):
    ...
    #np.savetxt('ASCADdata/loss_AE(MLP)_MLP_'+str(node)+'node_'+str(layer_nb)+'layers_'+str(input_dim)+'.txt',np_out)    
    #print("保存文件成功")
    aplt = plt.subplot()
...
input_dim = 100
ascad_database = "ASCAD_dataset/AE/ASCAD_AE_"+str(input_dim)+".h5"
...
for layer_nb in range(4,5):
    ...
    for node in list_node:
        print("node:-----------------------------------"+str(node))
        training_model = "ASCAD_trained_models/AE/model_AE(MLP)_MLP_"+str(node)+"node_"+str(layer_nb)+"layers_"+str(input_dim)+".h5"
