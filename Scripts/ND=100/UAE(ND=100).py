...
labeled_file='./ASCAD_dataset/ASCAD_desync100_Sbox3_3000_test.h5'
...
def ae_cnn_model_2(encoding_dim = 60,kernal=4,kernal_size=100):
    input_shape = (3000,1)
    ...
    modelSummary = './asymetric_ASCAD_desync100_AE(CNN)_'+str(kernal)+'x'+str(kernal_size)+'layers_model_summary.txt'
    print(modelSummary)
...
for encoding_dim in dims:
    for kernal in kernal_number:
        for kernal_size in kernal_sizes:
            ...
            model_weights = file_path + '/ae_model/ASCAD_desync100_AE(CNN)_'+str(kernal)+'x'+str(kernal_size)+'layers_model_weights.h5'  
            callbacks_list = [
            ...
            encoder,autoencoder,decoder = ae_cnn_model_2(encoding_dim,kernal,kernal_size)
            ...
            loss_file = file_path +'/loss_ASCAD_AE(MLP)_'+str(encoding_dim)+'encoding_dim_'+str(layer_nb)+'layers.txt'
            np.savetxt(loss_file,np_out)    
            print("保存文件成功")
            #divide trace set
            subfile_name = file_path+'/ASCAD_desync100_Sbox3_3000_test_'+str(encoding_dim)+'_AE(kernal_'+str(kernal)+'x'+str(kernal_size)+').h5'
