...
def mlp(node,layer_nb,input_dim):
    model = Sequential()
...
def train_model(X_profiling, Y_profiling, model, save_file_name, epochs=150, batch_size=100,node=16,layer_nb=3,input_dim=60):
    ...
    np.savetxt('ASCAD_dataset/loss_AE(MLP)_MLP_'+str(node)+'node_'+str(layer_nb)+'layers_'+str(input_dim)+'.txt',np_out)    
    print("file saved successfully!")
    aplt = plt.subplot()
    aplt.plot(history.history['acc'])
...
def check_model(model_file, ascad_database, num_traces=2000,node=16,layer_nb=3,input_dim=60):
    check_file_exists(model_file)
    ...
    ranks = full_ranks(model, X_attack, Metadata_attack, 4000, num_traces, 10)
    # We plot the results
    ...
    plt.grid(True)
    plt.plot(x, y)
    plt.savefig('ASCAD_trained_models/AE/model_AE(MLP)_MLP_'+str(node)+'node_'+str(layer_nb)+'layers_'+str(input_dim)+'_rank.png')
    ...
    path = 'ASCAD_trained_models/AE/model_AE(MLP)_MLP_'+str(node)+'node_'+str(layer_nb)+'layers_'+str(input_dim)+'_rank.txt'
    ...
    history_2 = np.loadtxt('ASCAD_trained_models/AE/model_AE(MLP)_MLP_'+str(node)+'node_'+str(layer_nb)+'layers_'+str(input_dim)+'_rank.txt')
    iteration2 = [n for n in range(history_2.shape[1])]
    history_list2 = history_2.tolist()
    x = history_list2[0]
    y = history_list2[1]
    print(x)
    print(y)
...
# change MLP model parameter here

input_dim = 75
ascad_database = "ASCAD_dataset/ASCAD_Sbox3_3000_test_AE.h5"
...
for layer_nb in range(4,5):
    ...
    for node in list_node:
        print("node:-----------------------------------"+str(node))
        training_model = "ASCAD_trained_models/AE/ASCAD_Sbox3_3000_test_AE.h5"
        model = mlp(node,layer_nb,input_dim)
        train_model(X_profiling, Y_profiling, model, training_model, epochs, batch_size,node,layer_nb,input_dim)
        #check model
        check_model(training_model, ascad_database, num_traces,node,layer_nb,input_dim)
